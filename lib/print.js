...
/**
 * An alias the can be used to refer to a package. For example, the 'toposort@2.0.3' can
...
class Package {
    ...
    return new Package({
      ...
      modules: [
        ...Object.keys(dependencies).map(key =>
          Key.key(key, dependencies[key])
        ),
        ...Object.keys(optionalDependencies).map(key =>
          Key.key(key, optionalDependencies[key])
        ),
      ].sort(),
    });
  }
  ...
  renderKey(asNixName = false) {
    ...
    return !asNixName
      ...
      : keys
          .map(k => k.id.toString().replace('@', '').replace('/', '-'))
          .join('-');
  }
        ...
        self: super: {
          "${this.renderKey()}" = super."${this.renderKey()}".override (x: {
            ...
            src = builtins.path {
              ...
              # Filter out node_modules folders form the package's soruce
...
function hostDependencyCycles(tree) {
  ...
  function hostCycle(cycle) {
    ...
    if (cycle.length === 2) {
      ...
      parent.modules[parent.modules.indexOf(child.renderKey())] =
        hostedChild.renderKey();
...
function resolveRef(ref, tree) {
  ...
  if (refOrPkg === undefined) {
    ...
    const found = Object.keys(tree).filter(
      n => n.startsWith(`${name}@`) && !Reference.is(tree[n])
    );
    if (isPath(version)) {
      ...
      assert.equal(
        found.length,
        1,
        'Found more that one local package with the same name'
      );
      ...
      // semver
      const availableCoercedVersions = found
        .map(parse)
        .map(({ version: v }) => semver.coerce(v).version);
      const resolvedVersion = semver.minSatisfying(
...
module.exports = async function print({ lock, out }) {
  /** @type YarnLockJSON */
  const { type, object: parsed } = lockfile.parse(
